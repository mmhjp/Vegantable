...
-->
	...
	<div class="col-md-10" >
		...
		<!-- sub title -->
		<h2 class="bd-title border-bottom pb-3 mb-3">자유게시판</h2>

		<!-- 검색입력 -->
		<div
			class="row mb-3 flex-nowrap justify-content-between align-items-center">
			<div class="col-md-2">
				<select class="form-select" id="searchKey" name="searchKey">
					<option value="">전체</option>
					<option value="post_title"
						<c:out value="${paramMap.searchKey eq 'post_title' ? 'selected':'' }"/>>제목</option>
					<option value="post_content"
						<c:out value="${paramMap.searchKey eq 'post_content' ? 'selected':'' }"/>>내용</option>
				</select>
			</div>
			...
			<div class="col-sm-6">
				<input type="text" class="form-control" id="searchKeyword"
					...
					value="<c:out value="${paramMap.searchKeyword }"/>">
			</div>
			...
			<div class="col-sm-2 d-flex justify-content-end align-items-center">
				<a href="<c:url value="/sample2/form.do"/>"
					class="btn btn-md btn-primary">글쓰기</a>
			</div>
		</div>
        ...
        ajaxForm('get', '/sample2/api-sample-list', formData, '', function(result) {
               ...
               if (getSample2List.length <= 0){
                   ...
                   $.each(getSample2List, function(key, getSample2Info) {
                       ...
                       tbodyHtml+= '    <td>'+getSample2Info.regist_id+'</td>';
                       tbodyHtml+= '</tr>';
                   });
               }
